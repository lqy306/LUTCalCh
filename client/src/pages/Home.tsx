...
async function invoke<T>(command: string, args: Record<string, string>): Promise<T> {
  const desktopInvoke = (window as unknown as { __TAURI__?: { core?: { invoke?: (name: string, payload: Record<string, string>) => Promise<T> } } }).__TAURI__?.core?.invoke;
  if (!desktopInvoke) throw new Error(`Desktop API unavailable for ${command}`);
...
import { applyLocale, getStoredLocale, localizeElementText, LOCALES, translateLegacy, type Locale } from "@/i18n/index";
...
const DEFAULT_LUT_TITLE = "Custom LUT";
...
/* 固定文件加载：桌面端走 Tauri IPC 读取绝对路径，浏览器端回退到站点相对路径 fetch。
   工作流 JSON 的 filePath 留空表示回放时打开系统选择框，由用户重新选择。 */
async function loadWorkflowFile(filePath: string): Promise<File | null>
{
  const fileName = filePath.split(/[\\/]/).pop() || "fixed-file.cube";
  try
  {
  ...
  {
    /* 浏览器环境没有 Tauri IPC，继续尝试按站点路径 fetch。 */
  }
  try
  {
    const url = /^[a-z][a-z0-9+.-]*:\/\//i.test(filePath) || filePath.startsWith("/")
      ? filePath
      : new URL(filePath, import.meta.env.BASE_URL).toString();
...
export default function Home() {
  ...
  /* 回放结束后短暂抑制引擎轮询的状态消息，让「流程执行完成」保持可见。 */
  const engineMessageHoldRef = useRef(false);
  ...
  const renameWorkflow = (id: string, nextName: string) => {
    ...
    if (!name) return;
    persistWorkflows(savedWorkflows.map((item) => (item.createdAt === id ? { ...item, name } : item)));
    setMessage("已重命名流程");
  };
  const deleteWorkflow = (id: string) => {
    persistWorkflows(savedWorkflows.filter((item) => item.createdAt !== id));
    setMessage("已删除流程");
  };
  const moveWorkflow = (id: string, direction: -1 | 1) => {
    const index = savedWorkflows.findIndex((item) => item.createdAt === id);
    const next = index + direction;
    if (index < 0 || next < 0 || next >= savedWorkflows.length) return;
    const updated = [...savedWorkflows];
    [updated[index], updated[next]] = [updated[next], updated[index]];
    persistWorkflows(updated);
  };
  const renameProfile = (id: string, nextName: string) => {
    const name = nextName.trim();
    if (!name) return;
    persistProfiles(profiles.map((item) => (item.id === id ? { ...item, name } : item)));
    setMessage("已重命名配置");
  };
  ...
  const moveProfile = (id: string, direction: -1 | 1) => {
    ...
    const next = index + direction;
    if (index < 0 || next < 0 || next >= profiles.length) return;
  ...
  const enforceAdjustmentEmbedLayout = useCallback(() => {
    ...
    const hiddenSelectors = ["#titlebar", "#footer", "#printable", "#shed", "#javascriptwarning", "#box-cam", "#box-gam", "#mob-status"];
  ...
  const refreshPreview = useCallback(() => {
    ...
    if (!context) return;
    /* stop 图表画布背景透明，深色主题下呈黑色；统一先铺白底保证曲线/网格/文字可辨识。 */
  ...
  /* 预览在 iframe 内运行；将其 Canvas 快照同步至工作台，而不是用独立静态图片替代。 */
  ...
  const hydrateEngine = useCallback(() => {
    const documentRef = engineDocument();
    if (!documentRef) return;
    /* 引擎 iframe 必须包含计算工作区结构才算加载成功；404/错误页不得误报“已连接”。 */
    ...
    if (!hasEngineStructure)
    {
      ...
      if (engineFailureCountRef.current >= 10 && !engineFailedRef.current)
      {
        ...
        setMessage("计算引擎加载失败：请检查网络连接或刷新页面重试");
      }
      return;
    }
    ...
    if (engineFailedRef.current)
    {
      ...
      if (!isReplayingRef.current && !engineMessageHoldRef.current) setMessage("计算引擎已就绪");
    }
    /* 工作流控件 id 使用单调递增编号：避免引擎动态增删控件后按文档位置
       重复分配（同一 id 命中两个控件），保证同一会话内 id 唯一且核心控件稳定。 */
    const existingIds = Array.from(documentRef.querySelectorAll("[data-lutcalc-workflow-id]"))
      .map((node) => Number((node.getAttribute("data-lutcalc-workflow-id") || "").replace(/\D/g, "")) || 0);
    let nextWorkflowId = Math.max(0, ...existingIds) + 1;
    documentRef.querySelectorAll("select, input, textarea, button, [role=button]").forEach((control) => {
      if (!control.getAttribute("data-lutcalc-workflow-id")) control.setAttribute("data-lutcalc-workflow-id", `lc-${String(nextWorkflowId++).padStart(4, "0")}`);
    });
    ...
    /* 计算引擎的默认标题是中文展示值；在非中文语言下写回相应语言包文本。 */
    if (nextState.title === "自定义 LUT" || nextState.title === "Custom LUT")
    {
      nextState.title = DEFAULT_LUT_TITLE;
      const titleNode = fieldNode("title");
      const windowRef = engineWindow();
      if (titleNode && windowRef && titleNode.value !== DEFAULT_LUT_TITLE)
      {
        titleNode.value = DEFAULT_LUT_TITLE;
  ...
  const recordEvent = useCallback((event: Event) => {
    ...
    /* 点击 select / 文本框 / 文件框只是打开控件，不改变任何状态，由随后的 change 记录最终值。 */
    if (isClick && (tag === "select" || tag === "textarea" || type === "text" || type === "file")) return;
    const isToggle = type === "checkbox" || type === "radio";
    const action: WorkflowEvent["action"] = isClick ? "click" : "change";
    ...
    setEvents((current) =>
    {
      /* 同类事件按选择器就地去重：只保留用户对该控件的最终操作，
         避免重复 change / 重复点击把流程录成“录屏”；首次出现顺序保持不变。 */
      const index = current.findIndex((item) =>
      {
        if (item.action !== action || item.selector !== next.selector) return false;
        if (!isClick || isToggle) return true;
        return item.value === next.value && item.checked === next.checked;
      });
      if (index === -1) return [...current, next].slice(-100);
  ...
  /*
   * LUT 分析器 的文件读取是异步的：change 事件只代表文件已选中，
  ...
  const captureCompletedLutAnalysis = useCallback((finalAttempt = false) =>
  {
    ...
    if (outputGamma.startsWith("LA - ") && outputGamut.startsWith("LA - "))
    {
      ...
      setLutAnalysis((current) => ({ ...current, status: "ready", title, outputGamma, outputGamut, completedAt: new Date().toLocaleTimeString("zh-CN"), message: "LUT 分析器 已完成分析，结果已成为当前输出管线。", samples }));
    ...
    {
      analysisInProgressRef.current = false;
      setLutAnalysis((current) => ({ ...current, status: "error", message: "计算引擎未注册 LA 输出；默认 S-Log3 状态不会被误报为分析成功。请检查 LUT 格式、范围和输入基底。" }));
  ...
  /* 原版 03 输出区大部分通过无 id 的 radio/select 实现；按既有 name/value 精确桥接。 */
  ...
  {
    if (!file) return;
    const mimeExt: Record<string, string> = { "image/jpeg": "jpg", "image/png": "png", "image/bmp": "bmp" };
    const suppliedExt = file.name.includes(".") ? file.name.slice(file.name.lastIndexOf(".") + 1).toLowerCase() : "";
    const inferredExt = mimeExt[file.type] || "";
    const validExt = ["jpg", "jpeg", "png", "bmp"].includes(suppliedExt) ? suppliedExt : inferredExt;
    if (!validExt) {
      setMessage("载入预览仅支持 JPG、JPEG、PNG 或 BMP 文件");
      return;
    }
    const engineFile = suppliedExt === validExt
      ? file
      : new File([file], `${file.name}.${validExt}`, { type: file.type || `image/${validExt === "jpg" ? "jpeg" : validExt}` });
    const reader = new FileReader();
    reader.onload = () => setPreviewImageSrc(String(reader.result || ""));
    reader.readAsDataURL(engineFile);
    const documentRef = engineDocument();
    const windowRef = engineWindow();
    const engineInput = documentRef?.querySelector("#preview-holder > input[type=file]") as HTMLInputElement | null;
    if (!engineInput || !windowRef) {
      setMessage("预览图片已读取，但预览引擎尚未准备好，请先显示预览后重试");
      return;
    }
    try {
      const transfer = new DataTransfer();
      transfer.items.add(engineFile);
      engineInput.files = transfer.files;
      const changeEvent = new (windowRef as unknown as { Event: typeof Event }).Event("change", { bubbles: true });
      engineInput.dispatchEvent(changeEvent);
      const previewInstance = (windowRef as unknown as { lutPreview?: { preGetImg?: () => void } }).lutPreview;
      previewInstance?.preGetImg?.();
      setMessage(`已载入预览图片：${file.name}；请确认 Gamma、色彩空间和范围`);
      window.setTimeout(refreshEnginePreview, 500);
      window.setTimeout(refreshEnginePreview, 1200);
      window.setTimeout(refreshPreview, 900);
    } catch {
      setMessage("预览图片已读取，但无法交给预览引擎处理");
    }
  };
  ...
  const syncLutAnalystConfig = (control: string, value: string) =>
  {
    ...
    if (control === "title")
    {
      ...
      /* 标题字段 没有显式 type 属性；它是分析设置容器的第一个 input。 */
  ...
  const importAdjustmentLut = (file: File) =>
  {
    ...
    input.dispatchEvent(new (windowRef as unknown as { Event: typeof Event }).Event("change", { bubbles: true }));
    analysisInProgressRef.current = false;
    setLutAnalysis({ status: "loading", fileName: file.name, title: "", outputGamma: "", outputGamut: "", completedAt: "", message: "文件已载入，正在读取文件元数据与分析参数。", samples: [] });
  ...
  const analyzeAdjustmentLut = () =>
  {
    ...
    setLutAnalysis((current) => ({ ...current, status: button ? "analyzing" : "error", outputGamma: "", outputGamut: "", completedAt: "", message: button ? "正在由LUT 分析器 计算 Gamma、Gamut 与分析 LUT。" : "未找到 LUT 分析操作。", samples: [] }));
    button?.click();
  ...
  const syncOriginalPreview = (kind: "toggle" | "preset" | "range" | "scope" | "load", value?: string | boolean) =>
  {
    ...
    if (kind === "toggle")
    {
      const previewButton = Array.from(documentRef.querySelectorAll("input[type=button], button")).find((node) => {
        const input = node as HTMLInputElement;
        const text = `${input.value || ""} ${node.textContent || ""}`;
        return /Preview|预览/.test(text) && !/Load Preview|载入预览/.test(text);
      }) as HTMLInputElement | undefined;
      if (previewButton) {
        previewButton.click();
        setPreviewVisible((current) => !current);
        setMessage(previewVisible ? "正在隐藏预览" : "正在显示预览");
      } else {
        setMessage("未找到预览切换按钮");
      }
    }
    else if (kind === "preset")
    {
    ...
    [260, 900, 1600, 2600, 4200].forEach((delay) => { window.setTimeout(refreshEnginePreview, delay); window.setTimeout(refreshPreview, delay); });
  };

  const saveWorkflow = () => {
    if (!events.length) { setMessage("请先调整参数，再保存流程"); return; }
    const workflow: WorkflowFile = { version: 1, name: workflowName.trim() || DEFAULT_WORKFLOW_NAME, createdAt: new Date().toISOString(), events };
    persistWorkflows([workflow, ...savedWorkflows.filter((item) => item.name !== workflow.name)]);
    setMessage(`已保存 ${events.length} 个步骤`);
  };
  const exportWorkflow = (workflow: WorkflowFile = { version: 1, name: workflowName || DEFAULT_WORKFLOW_NAME, createdAt: new Date().toISOString(), events }) => {
  ...
  const replayWorkflow = async (workflow: WorkflowFile) => {
    ...
    try {
      for (const step of workflow.events) {
        /* 文件步骤按记录标签路由到 LUTAnalyst 的文件输入：引擎 DOM 顺序在不同会话
           可能变化导致选择器失效，而文件值（fakepath / filePath）只属于这一个文件框；
           按钮点击的 label 也可能包含「导入新的 LUT」字样，因此只对 change 文件步骤路由。 */
        const isLaFileStep = step.action === "change" && (Boolean(step.filePath) || /(^|[\\/])fakepath[\\/]/i.test(step.value || ""));
        const target = (isLaFileStep
          ? documentRef.querySelector("#box-twk input[type=file]")
          : documentRef.querySelector(step.selector)) as HTMLInputElement | HTMLSelectElement | HTMLButtonElement | null;
        if (!target) continue;
        const IframeEvent = (windowRef as unknown as { Event: typeof Event }).Event;
        ...
        else if (target.type === "file")
        {
          if (step.filePath)
          {
            /* 固定文件模式：JSON 中 filePath 指定路径，跳过系统选择框直接载入。 */
            setMessage(`流程执行中：载入固定文件 ${step.filePath}`);
            const file = await loadWorkflowFile(step.filePath);
            if (!file)
            {
              setMessage(`固定文件不可用：${step.filePath}`);
              return;
            }
            ...
            fileInput.dispatchEvent(new IframeEvent("change", { bubbles: true }));
          }
          else
          {
            ...
            /* 计算引擎读取文件是异步的，留出解析时间再继续。 */
            await new Promise((resolve) => window.setTimeout(resolve, 800));
          }
        }
        else
        {
          if (step.value !== undefined) target.value = step.value;
      ...
      setMessage(`流程执行中断：${error instanceof Error ? error.message : String(error)}`);
    } finally {
      isReplayingRef.current = false;
      engineMessageHoldRef.current = true;
      window.setTimeout(() => { engineMessageHoldRef.current = false; }, 3000);
    }
  };
  ...
  const previewHint = useMemo(() => engineReady ? "预览由计算引擎实时生成" : engineFailed ? "计算引擎加载失败；预览不可用" : "正在读取计算引擎…", [engineFailed, engineReady]);
    ...
    <main className="apple-app-shell" aria-label="LUTCalc 中文计算器工作台">
      <header className="apple-topbar">
        ...
        <div className="topbar-actions">
          <label className="locale-controls" aria-label="Language"><span>Language</span><select value={locale} onChange={(event) => setLocale(event.target.value as Locale)}>{LOCALES.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
      ...
      <div className="apple-workspace">
        ...
        <section className="native-calculator-pane">
          ...
          <div className="native-calculator-grid">
            <div className="left-workbench-column">
            ...
            <section className={`native-card preview-card ${previewVisible ? "is-open" : "is-closed"}`}>
              ...
              <div className="preview-tool-bar">
                ...
                <select aria-label="预览类型" value={previewPreset} onChange={(event) => syncOriginalPreview("preset", event.target.value)}><option value="high">高对比度</option><option value="low">低对比度</option><option value="rec709">Rec.709 色域</option><option value="chromaticity">xy / uv 色度图</option><option value="gray">灰度</option></select>
              ...
              <div className="preview-content">
                <div className={`engine-preview-surface ${rgbSamplerEnabled ? "is-sampling" : ""}`} onMouseMove={updatePreviewRGB} onClick={samplePreviewPixel} role={rgbSamplerEnabled ? "button" : undefined} tabIndex={rgbSamplerEnabled ? 0 : undefined}>
                  {enginePreviewSrc || previewImageSrc ? <img src={enginePreviewSrc || previewImageSrc} alt="LUTCalc Canvas 预览" /> : <div className="preview-placeholder"><Eye size={22} />点击“显示预览”后读取Canvas</div>}
              ...
              {(previewScope.wfm || previewScope.vector || previewScope.rgb) && <div className="engine-scope-grid">{previewScope.wfm && <div>{engineScopeSrc.wfm ? <img src={engineScopeSrc.wfm} alt="波形监看" /> : <span>正在生成 WFM</span>}</div>}{previewScope.vector && <div>{engineScopeSrc.vector ? <img src={engineScopeSrc.vector} alt="矢量示波器" /> : <span>正在生成 Vector</span>}</div>}{previewScope.rgb && <div>{engineScopeSrc.rgb ? <img src={engineScopeSrc.rgb} alt="RGB Parade" /> : <span>正在生成 RGB Parade</span>}</div>}</div>}
              ...
              <div className="preview-footnote"><span>状态</span><strong>{engineReady ? "Canvas 已桥接" : engineFailed ? "引擎不可用" : "加载中"}</strong><span>鼠标移动可在预览中读取 10-bit RGB；载入图片会要求确认 Gamma、色彩空间与范围。</span></div>
            </section>
            </div>
          </div>
        </section>
      ...
      <footer className="project-disclosure" aria-label="关于与许可">
        <details>
          ...
          <div className="project-disclosure-body">
            <p>{translateLegacy(locale, "LUTCalc 新版本由 AI 维护，提供独立的色彩科学工作台体验。")}</p>
            <p>{translateLegacy(locale, "本软件采用 GNU GPL-2.0 许可；详情请参阅许可证条款。")}</p>
