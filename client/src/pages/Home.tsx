...
const adjustmentLabels = ["WHITE BALANCE", "PSST-CDL", "ASC-CDL", "MULTITONE", "HIGHLIGHT GAMUT", "KNEE", "BLACK LEVEL", "BLACK GAMMA", "DISPLAY CONVERTER", "GAMUT LIMITER", "FALSE COLOUR", "LUTANALYST"];

function optionsFrom(select: HTMLSelectElement | undefined): SelectOption[] {
  return select ? Array.from(select.options).map((option) => ({ value: option.value, label: option.text })) : [];
}
...
export default function Home() {
  ...
  const [title, setTitle] = useState("Custom LUT");
  ...
  return <main className="app-shell">
    <header className="topbar"><div className="brand-lockup"><div className="brand-mark" aria-hidden="true"><img src="/manus-storage/lutcalc-mark_dfa134db.png" alt="" /></div><div><p className="eyebrow">COLOR SCIENCE / WORKBENCH 04</p><div className="brand-name">LUT<span>Calc</span></div></div></div><div className="topbar-meta"><span className="status-dot" /><span>{engineReady ? "Local processing · ready" : "Loading engine · standby"}</span><button className="icon-button" onClick={() => setShowGuide((value) => !value)} aria-label="Toggle guide" title="显示使用提示"><PanelRight size={18} /></button></div></header>
    <section className="calibration-bar" aria-label="Calibration status"><span className="calibration-label">01 / SIGNAL PATH</span><div className="calibration-line"><i /><i /><i /><i /><i /><i /><i /></div><span className="calibration-value">INPUT → WORKING → OUTPUT</span></section>
    <div className="workspace-heading"><div><p className="section-kicker"><FlaskConical size={14} /> FILM LAB INTERFACE</p><h1>把输入信号整理成<br /><em>可控的画面。</em></h1><p className="lede">原生参数卡直接连接 LutCalc 的计算引擎，让每一次校准都更容易读懂、检查和导出。</p></div><div className="heading-actions"><a className="text-link" href={LUTCALC_URL} target="_blank" rel="noreferrer"><ExternalLink size={15} /> Open standalone</a><span className="version-chip">v4.09 / GPL-2.0</span></div></div>

    <section className="workbench-grid">
      <aside className="project-rail"><div className="rail-label"><Menu size={15} /> PROJECT RAIL</div><div className="rail-card active"><span className="rail-index">A</span><div><strong>{title || "Untitled grade"}</strong><small>React parameter session</small></div><span className="rail-live">{engineReady ? "LIVE" : "LOAD"}</span></div><div className="rail-divider" /><div className="rail-note"><SlidersHorizontal size={15} /><span>原生卡片通过同源桥接驱动原始 LUTCalc 引擎，计算仍在浏览器本地完成。</span></div><div className="rail-bottom"><span>Source engine</span><strong>cameramanben / LUTCalc</strong></div></aside>

      <section className="calculator-stage"><div className="stage-head"><div><span className="module-tag">02</span><div><p className="stage-title">LUT generator</p><p className="stage-caption">原生参数卡 · 输入、工作空间与输出</p></div></div><div className="stage-tools"><span className="small-status"><span className="status-dot" /> {engineReady ? "synced" : "loading"}</span><a href={LUTCALC_URL} target="_blank" rel="noreferrer" aria-label="Open calculator in new window"><FileDown size={17} /></a></div></div>
        <div className="native-workbench">
          <section className="param-card param-card-title"><div className="param-card-head"><span className="module-tag small">A1</span><div><h2>Project identity</h2><p>命名导出的 LUT 文件</p></div></div><label className="param-field wide"><span>LUT TITLE / FILENAME</span><input value={title} onChange={(event) => syncTitle(event.target.value)} /></label><button className="dark-action" onClick={() => invoke("Auto Title")}>AUTO TITLE</button></section>
          <section className="param-card"><div className="param-card-head"><span className="module-tag small">A2</span><div><h2>Input signal</h2><p>相机与记录端参数</p></div></div><div className="param-grid"><ParamSelect label="CAMERA" value={engine.camera} options={cameraOptions} onChange={(value) => syncSelect("#box-cam", 0, value, "camera")} /><ParamSelect label="CAMERA MODEL" value={engine.model} options={modelOptions} onChange={(value) => syncSelect("#box-cam", 1, value, "model")} wide /><ParamNumber label="NATIVE / CINEEI ISO" value={engine.nativeIso} onChange={(value) => syncNumber(0, value, "nativeIso")} /><ParamNumber label="STOP CORRECTION" value={engine.stopCorrection} onChange={(value) => syncNumber(1, value, "stopCorrection")} /></div></section>
          <section className="param-card"><div className="param-card-head"><span className="module-tag small">A3</span><div><h2>Working transform</h2><p>输入 gamma / gamut 与输出转换</p></div></div><div className="param-grid"><ParamSelect label="REC GAMMA" value={engine.recGamma} options={recGammaOptions} onChange={(value) => syncSelect("#box-gam", 1, value, "recGamma")} /><ParamSelect label="REC GAMUT" value={engine.recGamut} options={recGamutOptions} onChange={(value) => syncSelect("#box-gam", 5, value, "recGamut")} /><ParamSelect label="OUT GAMMA" value={engine.outGamma} options={outGammaOptions} onChange={(value) => syncSelect("#box-gam", 7, value, "outGamma")} /><ParamSelect label="OUT GAMUT" value={engine.outGamut} options={outGamutOptions} onChange={(value) => syncSelect("#box-gam", 11, value, "outGamut")} wide /></div></section>
          <section className="param-card"><div className="param-card-head"><span className="module-tag small">A4</span><div><h2>Output format</h2><p>尺寸、类型与裁切规则</p></div></div><div className="segmented-control"><span className="param-label">LUT DIMENSION</span><div>{[["1D", "1d", 0], ["3D", "3d", 1]].map(([label, value, index]) => <button key={String(value)} className={lutMode === value ? "selected" : ""} onClick={() => syncRadio("dims", Number(index), String(value), "lutMode")}>{label}</button>)}</div></div><div className="segmented-control"><span className="param-label">GRID SIZE</span><div>{[["17", 0], ["33", 1], ["65", 2]].map(([value, index]) => <button key={String(value)} className={dimension === String(value) ? "selected" : ""} onClick={() => syncRadio("dimension", Number(index), String(value), "dimension")}>{value}³</button>)}</div></div><div className="param-grid"><ParamSelect label="LUT FORMAT" value={engine.lutFormat} options={options.format ?? []} onChange={(value) => syncLabeledSelect("LUT Type", value, "lutFormat")} /><ParamSelect label="HARD CLIP" value={engine.hardClip} options={options.clip ?? []} onChange={(value) => syncLabeledSelect("Hard Clip", value, "hardClip")} /></div></section>
          <section className="param-card adjustments-card"><div className="param-card-head"><span className="module-tag small">A5</span><div><h2>Adjustments</h2><p>按需启用原始 LutCalc 的精调模块</p></div></div><div className="adjustment-grid">{adjustmentLabels.map((label, index) => <label key={label} className={`adjustment-toggle ${adjustments[index] ? "active" : ""}`}><input type="checkbox" checked={Boolean(adjustments[index])} onChange={(event) => syncAdjustment(index, event.target.checked)} /><span>{label}</span></label>)}</div></section>
          <div className="native-actions"><button className="secondary-action" onClick={() => invoke("Preview")}><Play size={14} /> PREVIEW</button><button className="primary-action" onClick={() => invoke("Generate LUT")}>GENERATE LUT <ExternalLink size={14} /></button><button className="secondary-action" onClick={() => invoke("Generate Set")}>GENERATE SET</button></div>
        </div>
        <iframe ref={engineRef} className="engine-frame" src={LUTCALC_URL} title="LUTCalc calculation engine" onLoad={() => setEngineReady(true)} />
        <div className="output-strip"><div><span className="output-mark" /> <span className="output-label">OUTPUT STATION</span><strong>{engineReady ? "React controls synced to the LUTCalc engine." : "Waiting for calculation engine."}</strong></div><a href={LUTCALC_URL} target="_blank" rel="noreferrer">GENERATE IN FULL WINDOW <ExternalLink size={13} /></a></div>
      </section>

      <aside className={`preview-rail ${showGuide ? "guide-open" : ""}`}><div className="preview-image" style={{ backgroundImage: "url('/manus-storage/lutcalc-preview-grid_79c2c30e.jpg')" }}><div className="preview-overlay"><Sparkles size={15} /><span>PREVIEW / TEST CHART</span></div></div><div className="preview-copy"><span className="module-tag">03</span><div><p className="stage-title">Preview monitor</p><p className="stage-caption">在生成 LUT 前检查信号走势</p></div></div>{showGuide ? <div className="guide-card"><strong>快速提示</strong><p>先选相机与输入 gamma，再确认工作色域和输出格式。React 参数卡会将变化实时同步到原始计算引擎。</p></div> : <div className="signal-bars"><span>R</span><i className="bar red" /><span>G</span><i className="bar green" /><span>B</span><i className="bar blue" /><span>Y</span><i className="bar yellow" /></div>}<div className="preview-foot"><span>TEST IMAGE</span><span>RGB / 16 bit</span></div></aside>
    </section>
