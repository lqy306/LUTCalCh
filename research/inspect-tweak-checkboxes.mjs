...
const response = await command('Runtime.evaluate', { expression: `(() => { const d = document.querySelector('iframe')?.contentDocument; return Array.from(d?.querySelectorAll('#box-twk input.twk-checkbox, #box-twk input.twk-checkbox-hide') || []).map((node, index) => ({index, checked:node.checked, className:node.className, parent:node.parentElement?.textContent?.trim().replace(/\\s+/g,' ').slice(0,100), grand:node.parentElement?.parentElement?.textContent?.trim().replace(/\\s+/g,' ').slice(0,130)})); })()`, returnByValue: true });
socket.close(); console.log(JSON.stringify(response.result.value, null, 2));
